...
internal sealed class ProjectBuilder : IAsyncDisposable
{
    ...
    public ProjectBuilder(PackageFixture fixture, ITestOutputHelper testOutputHelper, string defaultSdkName)
    {
        ...
        // Packages generated by the tests must only be resolved from the test source, so a package with the same
        // identity on nuget.org is never restored
        var testPackagePatterns = string.Join("\n            ", TestPackages.PackageIds.Select(id => $"""<package pattern="{id}" />"""));
